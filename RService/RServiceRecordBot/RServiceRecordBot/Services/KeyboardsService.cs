...
namespace RServiceRecordBot.Services
{
    internal static class KeyboardsService
    {
        ...
        public static InlineKeyboardMarkup GetRecordCollection(List<MenuElement> elements, string name)
        {
            var byttons = elements.Select(e => new[] { InlineKeyboardButton.WithCallbackData(text: e.Text, callbackData: $"{name}={e.Id}") }).ToArray();
            return new(byttons);
        }
        ...
        public static InlineKeyboardMarkup CancalRecord(string id)
        {
            return new(new[] { InlineKeyboardButton.WithCallbackData(text: "Отменить запись", callbackData: $"cancel_record={id}"),
                               InlineKeyboardButton.WithCallbackData(text: "Назад", callbackData: $"my_records") });
        }
    }
}
